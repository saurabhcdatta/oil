...
for (v in Y_VARS) {
  ...
  nn_fit <- tryCatch(
    neuralnet(
      ...
      linear.output = TRUE,       # regression, not classification
    ...
    error = function(e) {
      msg("  NN failed for %s: %s — using fallback (linear)", v, e$message)
      NULL
    }
  )

  if (is.null(nn_fit)) {
    # Fallback: simple linear model (lm) if neuralnet fails
    nn_models[[v]] <- lm(as.formula(paste0(y_std_col, " ~ .")),
                          data=train_df)
    nn_models[[v]]$model_type <- "lm_fallback"
  } else {
    nn_models[[v]] <- nn_fit
    nn_models[[v]]$model_type <- "neuralnet"
  }
  ...
  nn_perf_dt[[v]] <- data.table(
    outcome    = v,
    model_type = nn_models[[v]]$model_type,
    rmse_test  = round(rmse, 5),
    r2_test    = round(r2,   4)
  )
  msg("  %s | R²=%.3f | RMSE=%.5f", v, r2, rmse)
}

perf_dt <- rbindlist(nn_perf_dt)
...
msg("\nModels saved → Models/04c_nn_models.rds")
...
msg("Running Kernel SHAP: %d obs × %d outcomes × %d coalitions ...",
    nrow(X_shap_mat), length(Y_VARS), N_COAL)
msg("(Expected runtime: ~5–15 minutes depending on hardware)")

shap_results <- list()

for (v in Y_VARS) {
  msg("  SHAP: %-30s", v)
  sv_mat <- matrix(NA_real_, nrow=nrow(X_shap_mat), ncol=length(feature_cols),
                   dimnames=list(NULL, feature_cols))
  for (i in seq_len(nrow(X_shap_mat))) {
    sv_mat[i, ] <- compute_shap(X_shap_mat[i, ], v, n_coal=N_COAL)
    if (i %% 20 == 0) cat(sprintf("    obs %d/%d\n", i, nrow(X_shap_mat)))
  }
  shap_results[[v]] <- sv_mat
}
...
if (length(oil_col_idx) == 1) {
  ...
  x_base <- colMeans(X_test_mat)  # hold everything else at test mean

  msg("Computing partial dependence over %d oil price levels ...",
      length(oil_range_raw))

  pdp_rows <- lapply(seq_along(oil_range_raw), function(ri) {
    ...
    for (v in Y_VARS) {
      y_hat_std <- predict_std(v, x_perturb)
...
cat("\n", strrep("=",70), "\n")
cat("  Script 04c complete — neuralnet backend, no external dependencies\n")
cat("  All packages available from CRAN, no torch/TensorFlow required\n")
cat(strrep("=",70), "\n")
