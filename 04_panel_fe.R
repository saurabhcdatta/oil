...
keep_patterns <- c(
  ...
  # Dep vars — new
  "^member_growth_yoy$","^pll_rate$","^pll_per_loan$",
...
#
# INTERPRETATION BY DEP VAR:
#   dq_rate          — oil bust → realised delinquency (lagged, backward-looking)
#   pll_rate         — oil bust → forward provisions (leading indicator, proactive mgmt)
#   netintmrg        — oil-driven rate cycle → NIM compression/expansion
#   insured_share_growth — oil income → deposit inflows (buffer hypothesis)
#   member_growth_yoy — oil shock → membership gains/losses (early warning signal)
#   costfds          — oil → inflation → Fed hikes → CoF squeeze
#   cert_share       — rate hike → certificate migration (deposit repricing)
#   loan_to_share    — oil-state economic activity → loan demand differential
...
# main_dvars: dep vars for which we run subsample models + save full tables
# Includes the two new variables alongside the original four core outcomes
main_dvars  <- intersect(c("dq_rate","netintmrg","insured_share_growth",
                             "costfds","pll_rate","member_growth_yoy"),
                          dep_vars)
    ...
    if (length(mod_list) > 0) {
      ...
      tryCatch({
        modelsummary(mod_list,
                     ...
                     output = tbl_path,
                     title  = paste("Panel FE:", dep_labels[v] %||% v))
...
cat("    04a_02_direct_indirect_decomp  beta1/beta2/beta3 decomposition\n")
cat("    04a_03_structural_break_coefs  Pre vs post shale beta\n")
cat("    04a_04_fomc_interaction.png    FOMC regime interaction\n")
cat("    04a_05_r2_progression.png      R2 by model spec\n\n")
cat("  Tables/\n")
cat("    04a_fe_{dep_var}.txt           Publication tables\n\n")
cat("  Dependent variables estimated:\n")
cat("    dq_rate              Delinquency rate (backward-looking credit quality)\n")
cat("    pll_rate             PLL rate (forward-looking provisions / avg loans)\n")
cat("    netintmrg            Net interest margin\n")
cat("    insured_share_growth Deposit growth (buffer hypothesis)\n")
cat("    member_growth_yoy    Membership growth (early warning indicator)\n")
cat("    costfds              Cost of funds (rate channel)\n")
cat("    cert_share           Certificate share (deposit migration)\n")
cat("    loan_to_share        Loan-to-share ratio\n")
cat("    pcanetworth          Net worth ratio (capital adequacy)\n\n")
cat("  Model sequence:\n")
cat("    M1: Baseline (direct oil exposure only)\n")
cat("    M2: Direct + Indirect + Spillover decomposition <- PRIMARY\n")
cat("    M3: M2 + FOMC regime interaction\n")
cat("    M4: M3 + post_shale structural break interaction\n")
cat("    M5: Full specification (all channels + asymmetry + lags)\n")
cat("    M6: Clean sample (ex-GFC, ex-COVID)\n")
cat("    M_oil/M_nonoil: Oil-state vs non-oil subsamples\n")
cat("    M_pre/M_post: Pre/post-2015 shale era subsamples\n")
cat("=================================================================\n")
