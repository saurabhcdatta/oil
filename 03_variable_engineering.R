...
phase1_vars <- c(
  ...
  "cert_growth_yoy","dep_growth_yoy","member_growth_yoy",
...
cu_yoy <- function(col) {
  # Named result column avoids fragile [[2]] positional extraction
  res <- panel[, .(val = {
    x    <- get(col)
    x_l4 <- shift(x, 4L)
...
lag_dvars <- intersect(c("dq_rate","chg_tot_lns_ratio","netintmrg",
                          ...
                          "cert_share","loan_to_share",
                          "member_growth_yoy",
                          "pll_rate"), names(panel))
...
# ── 3.25 Member growth change variables ───────────────────────────────────────
# Complements the YoY level: QoQ acceleration and longer-horizon change
# Economic use: membership deceleration (chg turning negative) is an early
# warning indicator that precedes deposit outflows by 1-2 quarters (see 02 EDA)
if ("member_growth_yoy" %in% names(panel)) {
  panel[, member_growth_chg  := member_growth_yoy - shift(member_growth_yoy, 1),
            by=join_number]   # acceleration: is growth speeding up or slowing?
  panel[, member_growth_lag4 := shift(member_growth_yoy, 4),
            by=join_number]   # level one year prior (for Chow / FE specifications)
  msg("  \u2713 member_growth_chg (QoQ acceleration), member_growth_lag4")
}

# ── 3.26 PLL rate derived variables ───────────────────────────────────────────
# pll_rate is constructed in 01_data_prep.R as pll / avg_lns_tot.
# Here we add:
#   pll_rate_chg — QoQ change in provision rate (acceleration signal)
#   pll_rate_yoy — 4Q change (captures annual provisioning cycle)
#   pll_dq_gap   — pll_rate minus dq_rate (forward/backward gap):
#                  positive = management provisioning AHEAD of realised losses
#                  negative = management under-provisioning (stress signal)
if ("pll_rate" %in% names(panel)) {
  panel[, pll_rate_chg := pll_rate - shift(pll_rate, 1L), by=join_number]
  panel[, pll_rate_yoy := pll_rate - shift(pll_rate, 4L), by=join_number]
  msg("  \u2713 pll_rate_chg (QoQ), pll_rate_yoy (4Q change)")
}
if (all(c("pll_rate","dq_rate") %in% names(panel))) {
  panel[, pll_dq_gap := fifelse(
    !is.na(pll_rate) & !is.na(dq_rate),
    pll_rate - dq_rate,
    NA_real_)]
...
agg_qtr <- panel[, lapply(.SD, mean, na.rm=TRUE),
                  ...
                  .SDcols=intersect(
                    c("dq_rate","dq_rate_chg","netintmrg","d_netintmrg",
                      ...
                      "member_growth_yoy","member_growth_chg",
                      "pll_rate","pll_rate_chg","pll_dq_gap",
...
# Core endogenous block: 8 main CU outcomes including membership and PLL
...
chow_vars <- intersect(
  c("dq_rate","pll_rate","netintmrg","costfds",
...
dist_vars <- intersect(c("dq_rate","pll_rate","netintmrg","insured_share_growth",
                          "costfds","member_growth_yoy"), names(panel))
...
model_vars <- list(
  ...
  dep_vars = intersect(
    c("dq_rate",
      ...
      "costfds","pcanetworth",
      "member_growth_yoy",
      "pll_rate"),           # forward-looking credit quality
    names(panel)),

  # Lagged dep vars (dynamic panel)
  dep_lags = paste0(rep(c("dq_rate","pll_rate","netintmrg","costfds",
                           "member_growth_yoy"), each=4),
                    "_lag", 1:4),
...
cat("  New variables added this script:\n")
cat("    member_growth_chg   QoQ acceleration of membership growth\n")
cat("    member_growth_lag4  Membership growth one year prior\n")
cat("    member_growth_yoy_lag1..4  Dynamic panel lags\n")
cat("    pll_rate_chg        QoQ change in PLL rate\n")
cat("    pll_rate_yoy        4Q change in PLL rate\n")
cat("    pll_dq_gap          pll_rate - dq_rate (provisioning lead indicator)\n")
cat("    pll_rate_lag1..4    Dynamic panel lags for PLL rate\n\n")
